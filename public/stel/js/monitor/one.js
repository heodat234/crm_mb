...
router.route("/", async function() {
    var HTML = await $.get(`${Config.templateApi}monitor/one/overview`);
    var model = {
...
router.route("/callin", async function(id) {
    var HTML = await $.get(`${Config.templateApi}monitor/one/callin`);
    var model = {
...
router.route("/callout", async function(id) {
    var HTML = await $.get(`${Config.templateApi}monitor/one/callout`);
    var model = {
        dataSource: new kendo.data.DataSource({
            serverAggregates: true,
            aggregate: [],
