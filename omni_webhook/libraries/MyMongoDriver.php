...
class MyMongoDriver {
    ...
    private function mongoarray_to_array($mongoarray){
        $result = array();
        foreach ($mongoarray as $key => $value){
            if(is_array($value)){
                $result[$key] = $this->mongoarray_to_array($value);
            }
            else if($value instanceof MongoInt64){
                $result[$key] = intval($value->value);
            }
            else{
                $result[$key] = $value;
            }
     ...
     * 	--------------------------------------------------------------------------------
     *
     * 	aggregate the documents based upon the passed parameters
     *
     * 	@usage : $this->mongo_db->aggregate('foo');
     */
    
    public function aggregate($collection = "",$returns=array(),$options=array()) {
        ...
        $documents = $this->db->{$collection}->aggregateCursor($pipeline,$options);
        $result = array();
        foreach ($documents as $document){
            $result[] = $this->mongoarray_to_array($document);
        }
    ...
    public function aggregate_pipeline($collection = "",$pipeline=array(),$returns=array(),$options=array()) {
        ini_set('mongo.long_as_object', true);
    ...
    public function aggregate_pipeline_usedisk($collection = "",$pipeline=array(),$returns=array()) {
        if (empty($collection)) {
            exit("In order to retreive documents from MongoDB, a collection name must be passed");
        }
        
        $cursor = new stdClass();
        $options = array("allowDiskUse" => true,"cursor"=>$cursor);
        $documents = $this->db->{$collection}->aggregateCursor($pipeline,$options);
