...
class Notification_get_conversation  {
    ...
    function __construct() {
        ...
        $this->omni_webhook_socket_url = $this->WFF->config->item('OMNI_WEBHOOK_SOCKET_URL') . '/api/v2/chat';
        $this->omni_webhook_noifi_createroom = $this->WFF->config->item('OMNI_WEBHOOK_SOCKET_URL') . '/api/v2/loadnewroom';
    ...
    private function addMsg($data) {
        ...
        $room = $this->WFF->mongo_db4x->where(array('to.user_id' => $sender_id, 'source' => $data['messages']['source']['type'], 'status' => 1))->getOne('chatGroups');
        ...
        if (!empty($room)) {
            ...
            $this->WFF->mongo_db4x->where(array('_id' => new mongoId($room_id)))->set(array('date_active' => time()))->update('chatGroups');
        } else {
            $room_id = '';
            
            //Nếu message không nằm ở nhóm chat nào thì kiểm tra page thuộc quản lý của user nào để add notifi
           $pageapps = $this->WFF->mongo_db4x->where(array('id' => $page_id))->getOne('pageapps');
            ...
            $room_group = $this->WFF->mongo_db4x->where(array('to.user_id' => $sender_id, 'source' => $data['messages']['source']['type'], 'status' => 1))->getOne('chatGroups');
            $room_id = $room_group["_id"]->{'$id'};
        }

        $room_update = $this->WFF->mongo_db4x->where(array("_id" => new MongoId($room_id)))->set("read_by", array())->update('chatGroups');
        ...
        $result = $this->WFF->mongo_db4x->insert('chatMessages', $message_data);
    ...
    private function addComment($data) {
        ...
        $room = $this->WFF->mongo_db4x->where(array('to.details.post_id' => $post_id, 'to.user_id' => $sender_id, 'source' => 'facebook', 'status' => 1))->getOne('chatGroups');
        if (!empty($room)) {
            $room_id = $room['_id']->{'$id'};
            //Lưu lại active mới nhất
            $this->WFF->mongo_db4x->where(array('_id' => new mongoId($room_id)))->set(array('date_active' => time()))->update('chatGroups');
            ...
            $supervisor = $this->WFF->mongo_db4x->where(array('_id' => new mongoId($group_id)))->getOne('chatGroup_Manager');
        ...
        $this->sendUrl($this->omni_webhook_socket_url, $message_data);
        $this->WFF->mongo_db4x->insert('chatMessages', $message_data);
    }
    ...
    private function addMsgEcho($data) {//Tin Nhắn được gởi từ page trên facebook
        try {
            ...
            if (isset($data['messages']['metadata']['id'])) {
                ...
                $message_info = $this->WFF->mongo_db4x->where(array('_id' => new mongoId($message_id)))->getOne('chatMessages');
            } else {
                $message_info = '';
            }
            ...
            if (empty($message_info)) {
                ...
                $room = $this->WFF->mongo_db4x->where(array('to.user_id' => $recipient_id, 'source' => $data['messages']['source']['type'], 'status' => 1))->getOne('chatGroups');
                if (!empty($room)) {
                    $room_id = $room['_id']->{'$id'};
                    //Lưu lại active mới nhất   
                    $this->WFF->mongo_db4x->where(array('_id' => new mongoId($room_id)))->set(array('date_active' => time()))->update('chatGroups');
    ...
    public function redirectNotify($notification_data) {
        ...
        if ($line == "livechat" && $type == "new_livechat_chat") {
            ...
            $data_views = $this->WFF->mongo_db4x->where(array("type" => "new_livechat_chat", "sender_id" => $sender_id))->getOne('chatNotifi');
            ...
            $data_room = $this->WFF->mongo_db4x->where(array("page_id" => $notification_data['page_id']))->getOne('chatGroups');

            $result = $this->WFF->mongo_db4x->insert('chatGroups', $room_array);
            $room_id = $result->{'$id'};
            ...
            fclose($f);
*/
            $this->WFF->mongo_db4x->insert('chatNotifi', $notification_data);
        ...
        if ($line == "livechat_remote" && $type == "new_livechat_remote") {
            ...
            $data_views = $this->WFF->mongo_db4x->where(array("type" => "new_livechat_remote", "sender_id" => $sender_id))->getOne('chatNotifi');
        ...
        if ($line == "facebook" && $type == "new_facebook_chat") {
            ...
            $data_views = $this->WFF->mongo_db4x->where(array("type" => "new_facebook_chat", "sender_id" => $sender_id))->getOne('chatNotifi');
        ...
        if ($type == "new_facebook_comment") {
            ...
            $data_views = $this->WFF->mongo_db4x->where(array("type" => "new_facebook_comment", "sender_id" => $sender_id))->getOne('chatNotifi');
        ...
        if ($type == "new_zalo_chat") {
            ...
            $data_views = $this->WFF->mongo_db4x->where(array("type" => "new_zalo_chat", "sender_id" => $sender_id))->getOne('chatNotifi');
    ...
    public function UpdateMesChuaPhanphoiBySenderId($room_id, $sender_id, $source) {
        $this->WFF->mongo_db4x->where(array("sender_id" => $sender_id, "room_id" => ''))->set(array('room_id' => $room_id, 'source' => $source))->update_all('chatMessages');
    }
    ...
    /*public function get_livechat_remote(){
        try{
            $pageapps = $this->WFF->mongo_db4x->get('livechat_remote_pageapps');
