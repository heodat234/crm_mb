...
class Callevent_model extends CI_Model {
    ...
    public function get_key($secret ) {
        ...
        $config = $this->mongo_private->where(array('secret_key' => $secret, 'deleted' => array('$ne' => TRUE)))->getOne($this->config);
    ...
    public function getAgentFullName($userextension){
        ...
        $user = $this->mongo_private->where(array('extension' => $userextension ))->getOne($user_collection);
    ...
    public function get_cdr($calluuid) {
       return $this->mongo_db->where(array('calluuid' => $calluuid))->get($this->cdr);
    }
