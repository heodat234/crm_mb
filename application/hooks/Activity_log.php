...
class Activity_log 
{
	...
	private $uri_string_exceptions = [
		"undefined","js","public","upload","page","api/v1/ping","api/v1/widget","template/nav","template/widget", 
    ...
    public function write() 
    {
        ...
        if($CI->load->is_loaded('session') && $CI->session->userdata("my_session_id") && $CI->config->item("record_activity")) {
	        ...
	        $_db = $CI->config->item("_mongo_db");
	        $CI->mongo_db->switch_db($_db);
	        ...
	        if($check_flag) {
	        	...
	        	$get = $CI->input->get() ? $CI->input->get() : null;
	        	$post = $CI->input->post() ? $CI->input->post() : null;
