...
Class Preference extends WFF_Controller {
	...
	private $sub = "";
	private $fields = array("theme", "language", "page_preloader", "ringtone", "avatar", "email","phone");
	...
	function detail($extension)
	{
        ...
        if(empty($response["page_preloader"])) $response["page_preloader"] = $this->config->item("template")["page_preloader"];
	...
	function update($extension)
	{
		...
		if(isset($data["page_preloader"])) $data["page_preloader"] = (bool) ($data["page_preloader"] == "true");
	...
	function upload_ringtone()
	{
		try {
			...
			if(strpos($allowed_types, strtolower($file_parts['extension'])) === FALSE) throw new Exception("Wrong file type. Only accept mp3 or wav");
