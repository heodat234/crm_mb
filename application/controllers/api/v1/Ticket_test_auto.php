...
Class Ticket_test_auto extends WFF_Controller {
	...
	function getTicketLogsById() {
	    ...
	    try {
            $response = $this->mongo_db->where(array('ticket_id' => $request['id']))->order_by(array('action_time' => 'desc'))->limit($request['logs_view'])->get(set_sub_collection('Ticket_Logs'));
    ...
    function create() {
	    try {
            ...
            $pheanstalk->useTube('newticket')->put(json_encode($data));
            print_r("TEST");
        }
        catch (Exception $e) {
            echo json_encode(array("status" => 0, "message" => $e->getMessage()));
