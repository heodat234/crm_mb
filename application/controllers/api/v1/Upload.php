...
Class Upload extends WFF_Controller {
	...
	function avatar($folder = "")
	{
		try {
			...
			//pre($path);
			if (!file_exists($path)) { 
				mkdir(PICTURE_PATH . $folder, 0777);
			}
			$file = $_FILES['file'];
	...
	function attachment()
	{
		try {
			...
			if (!file_exists($path)) { 
				mkdir(UPLOAD_PATH . $folder, 0777);
			}
			$file = $_FILES['file'];
			$file_parts = pathinfo($file['name']);
			$notallowed_types = 'php|sh|bash';
			$filesize = filesize($file['tmp_name']);
			if(strpos($notallowed_types, strtolower($file_parts['extension'])) !== FALSE) throw new Exception("Wrong file type. Only accept jpeg, png or gif");
			if($filesize > 1000000) throw new Exception("File too large");
			$new_file_name = str_replace([" ","/"], ["",""], $file['name']);
			$file_path = $path . $new_file_name;
