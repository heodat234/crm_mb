...
class Pbxevents extends CI_Controller {
    ...
    function write_log() {
        $test = $_GET;
        $test["createdAt"] = time(); //date("Ymd\THis");
...
//        $data['calltype']=$request['calltype'];
    ...
    private function hangup_process($request,$secret,$connector){
        ...
        $this->callevent_model->delete_cdr_realtime($data['calluuid'], $connector);
    }

    private function cdr_process($request,$secret, $connector){
        $data['workstatus']="Complete";
        ...
        if($cdr[0]['direction']=="inbound"){
            ...
            if($cdr[0]['direction']=="outbound"){
                $data['dnis']='02873053333';
            }
        }
    ...
    private function Sync_process($request,$secret,$connector){
            ...
            foreach($calluuidlist as $calluuid){
                ...
                curl_setopt_array($curl, array(
                  CURLOPT_URL => $connector["pbx_url"]."getcdr2.php?calluuid=".$calluuid['calluuid']."&secrect=$secret&version=4",
