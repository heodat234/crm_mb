		...
		<div id="side-form" style="width: 50%">
			...
			<div class="form-group">
                ...
                <div style="float: right; display: block; white-space: nowrap">
                    <label>
                        <input id="isAgentAssign" style="vertical-align: middle" type="checkbox" data-bind="checked: item.isAgentAssign, events: {click: isAgentAssignClick}" />
				...
				<input data-role="dropdownlist"
                    ...
                    data-filter="contains"
                    data-bind="value: item.assign, source: assignOptionAgent, visible: isAgentAssignHide" style="width: 100%;"/>
                <input data-role="dropdownlist"
                       data-value-primitive="true"
                       data-text-field="name"
                       data-value-field="id"
...
var ticketOption = <?= !empty($doc) ? json_encode($doc) : '{}' ?>,
	ticketItem = Object.assign({
		status: "Open",
...
var ticketObservable = {
	...
	save: function() {
		if((typeof this.item.assign === 'undefined' || this.item.assign === '' || this.item.assign === null) && (typeof this.item.assignGroup === 'undefined' || this.item.assignGroup === '' || this.item.assignGroup === null)) {
			notification.show("@Xin vui lòng chọn đối tượng phân công.@", 'error');
			return false;
		}
        $.ajax({
            url: ENV.vApi + "ticket/create",
            type: "POST",
...
kendo.bind("#right-form", kendo.observable(ticketObservable));
