...
Class Mongo_db{
	...
	*/
	...
	private function prepare()
	{
		...
		{
			if(empty($this->config[$this->activate]['version']))
			{
				show_error("Version missing from mongodb config group : {$this->activate}", 500);
			}
			else
			{
				$this->version = trim($this->config[$this->activate]['version']);
			}
