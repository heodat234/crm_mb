window.oscWidget = {
	...
	open: function(e){
		...
		iframeWin.postMessage({st_popup_status: 1}, 'http://115.146.126.84/worldchatboxbuilder');
	},
	isClose: function(e){
	...
	setCss: function(){
        ...
        link.type = 'text/css';
        link.rel = 'stylesheet';
        link.href  = 'http://115.146.126.84/assets/js/livechat/widget.css';
	...
	setIframe: function(id){
		...
		iframe.innerHTML  = '<iframe id="osc_widget" width="100%" height="100%" frameborder="0" src="http://115.146.126.84/worldchatboxbuilder?app_id='+id+'"></iframe>';
	...
	setData: function(){
		...
		var iframeWin = document.getElementById("osc_widget").contentWindow;
		iframeWin.postMessage({user_data:{name:window.oscWidget.user.name, phone:window.oscWidget.user.phone, email:window.oscWidget.user.email, address:window.oscWidget.user.address, properties:window.oscWidget.user.properties, }}, "http://115.146.126.84/worldchatboxbuilder");//?app_id=5c10714deb721dbf50ca3c82
	},
	init: function(data){
		this.id = data['token'];
...
console.log(window.oscWidget.isOpen());
console.log(window.oscWidget.isClose());
